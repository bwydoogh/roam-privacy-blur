...
- **Two configuration paths.** When loaded by Roam Depot, `onload({ extensionAPI })` reads/writes settings through `extensionAPI.settings` and renders the native `Privacy Blur` settings panel via `createSettingsPanel`. When loaded manually (e.g. via `roam/js` import or the test harness), `extensionAPI` is absent and settings come from the `manualOptions` argument to `onload`, stored in module-level `fallback*` variables. `readUnlockOptions` / `getStoredSetting` branch on which path is active. Any new setting must be wired through both paths plus the harness mock.
